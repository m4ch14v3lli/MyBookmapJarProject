...
public class InstrumentInfo extends InstrumentCoreInfo implements Serializable {
    ...
    /**
     * Delay value for the dataDelay field, if the delay is currently unknown, 
     * but it is known that the data is delayed.
     ...
     * This allows bookmap to translate level numbers to price that is shown 
     ...
     * per price unit instead of per tick (can be computed as tick value divided by pips)
     * 
     ...
     * this instrument is a cryptocurrency or is based on cryptocurrencies in some way
     */
    public final boolean isCrypto;
    
    /**
     * Used to distinguish local history with special modes on the per instrument basis 
     * (i.e. local history with delayed data vs live data)
     * Provider should set this field to specify special mode of this instrument,"DELAYED" for example
     * 
     * Note: it is recommended to not set this field for LIVE instruments for backward 
     * compatibility of historical servers, i.e. so the historical server returned "live" 
     * data to the user with the old client
     */
    ...
    /**
     * Delay in nanoseconds for the data of this instrument. If delay is currently unknown, 
     * but it is known that the data is delayed, then you should set this field to UNKNOWN_DELAY, 
     * to inform the Bookmap that the data is delayed (so the Bookmap can apply some restrictions 
     * for delayed data instruments).
     */
    ...
    @Deprecated
    public InstrumentInfo(String symbol, String exchange, String type, double pips, 
                         double multiplier, String fullName, boolean isFullDepth) {
        this(symbol, exchange, type, pips, multiplier, fullName, isFullDepth, 1.0);
    }
    ...
    public BuilderBase<?> toBuilder() {
        return new Builder()
            ...
            .setIsFullDepth(isFullDepth)
            .setSizeMultiplier(sizeMultiplier)
            .setIsCrypto(isCrypto)
            .setRecordingTag(recordingTag)
            .setIsApiProtected(isApiProtected)
            .setIsNbboSupported(isNbboSupported)
            .setDataDelay(dataDelay)
            .setRequestedSymbol(requestedSymbol);
    }
...
/**
 * Core information about the instrument
 */
abstract class InstrumentCoreInfo implements Serializable {
...
/**
 * Crypto-specific instrument information
 */
class InstrumentInfoCrypto extends InstrumentInfo {
    
    protected InstrumentInfoCrypto(BuilderBase<?> builder) {
        super(builder);
    }
    
    public static class CryptoBuilder extends BuilderBase<CryptoBuilder> {
        public CryptoBuilder() {
            setIsCrypto(true);
        }
        
        @Override
        protected CryptoBuilder self() {
            return this;
        }
        
        @Override
        public InstrumentInfoCrypto build() {
            return new InstrumentInfoCrypto(this);
        }
    }
}

/**
 * Annotation to mark add-ons that can receive unrestricted data
 */
@interface UnrestrictedData {
}